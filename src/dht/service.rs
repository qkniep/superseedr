...
struct DemandPlannerActionView {
    kind: &'static str,
    info_hash: Option<InfoHash>,
    demand_class: Option<DemandSliceClass>,
    slice_class: Option<DemandSliceClass>,
    peer_count: Option<usize>,
    total_peers: Option<usize>,
    unique_peers: Option<usize>,
    runtime_available: Option<bool>,
    runtime_ready_count: Option<usize>,
    stop_reason: Option<DemandSliceStopReason>,
}

impl DemandPlannerActionView {
    fn from_action(action: &DemandPlannerAction<'_>) -> Self {
        match action {
...
#[derive(Debug, Clone, Copy, Default)]
struct DemandPlannerEffectView {
    kind: &'static str,
    info_hash: Option<InfoHash>,
    demand_class: Option<DemandSliceClass>,
    slice_class: Option<DemandSliceClass>,
    selection_reason: Option<DemandSelectionReason>,
    stop_reason: Option<DemandSliceStopReason>,
    total_peers: Option<usize>,
    unique_peers: Option<usize>,
    peer_count: Option<usize>,
    lookup_count: Option<usize>,
    unique_added: Option<usize>,
    force: Option<bool>,
    parked: Option<bool>,
    finish_mode: Option<DemandFinishMode>,
}

impl DemandPlannerEffectView {
    fn from_effect(effect: &DemandPlannerEffect) -> Self {
        match effect {
...
fn demand_planner_monitor_enabled() -> bool {
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| env::var_os(DHT_PLANNER_MONITOR_ENV).is_some())
}

fn short_info_hash(info_hash: InfoHash) -> String {
    hex::encode(&info_hash.as_ref()[..4])
}

fn optional_info_hash_label(info_hash: Option<InfoHash>) -> String {
    info_hash.map(short_info_hash).unwrap_or_default()
}

fn trace_demand_planner_reduction(
...
fn trace_demand_planner_effect(stage: &'static str, effect: &DemandPlannerEffect) {
    if !demand_planner_monitor_enabled() {
        return;
    }
